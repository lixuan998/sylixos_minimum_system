...
realloc_symlink(/*in/out*/ iso9660_stat_t *p_stat, uint8_t i_grow)
{
  if (!p_stat->rr.i_symlink) {
    ...
    p_stat->rr.psz_symlink = (char *) cdio_calloc(1, i_max);
    ...
    if ( i_needed <= p_stat->rr.i_symlink_max)
      ...
      char * psz_newsymlink = (char *) cdio_calloc(1, 2*i_needed);
      ...
      cdio_free(p_stat->rr.psz_symlink);
...
/* Return a pointer to a internal cdio_free buffer */
static char *
_getbuf (void)
{
